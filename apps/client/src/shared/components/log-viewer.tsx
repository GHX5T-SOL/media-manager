      ...
      >
        {entries.length === 0 ? (
          ...
          entries.map((entry, i) => {
            const colors = resolveLevelColors(entry.level, colorScale);
            return (
              <div key={i} className="flex items-start gap-2 border-b border-border/20 px-3 py-1.5">
                <Circle
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border/40 bg-muted/10 px-3 py-2">
        ...
        {/* Inline search */}
        <div className="ms-auto flex items-center gap-1.5 rounded-md border border-border/40 bg-background px-2 py-1">
...
function LogEntryRow({
  ...
  return (
    <Collapsible
      ...
      className="flex flex-col border-b border-border/20 px-3 py-1.5 transition-colors hover:bg-muted/30"
    >
